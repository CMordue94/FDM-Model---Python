...
G1E_N=((([i if n_e_xc[i-1]<G1E and n_e_xc[i]>=G1E else Ne for i in range(G1S_N,Nn)])[0]) if G1 == "Y" else 0)                                                    
#Gap 2 Indicator
G2S_N=((([i for i in range(0,Ne) if n_e_xc[i]<=G2S and n_e_xc[i+1]>G2S])[0]) if G2 == "Y" else 0)
G2E_N=((([i if n_e_xc[i-1]<G2E and n_e_xc[i]>=G2E else Ne for i in range(G2S_N,Nn)])[0])-1 if G2 == "Y" else 0)                                                    
...
if PAC == "Y":                                           
    ...
    a=[i for i in range(1,Nn) if n_e_xc[i-1]<=PS and n_e_xc[i]>PS][0]               #These find where the part angled section starts and ends with respect to the nodes it actually applies to and adjusts the element length accordingly.
    b=[i if n_e_xc[i-1]<=PE and n_e_xc[i]>PE else Nn for i in range(a,Nn)][0]
    Pe_x=(Ls_A-n_x[a-1])/(b-a)
    ...
    for i in range(1,Nn):              ##!!Currently unsure if fully working, may be better to scrap the above and make the lement number equal to the point in change on probes!
