...
if VM == "Y":
    ...
    Le_xa=np.asarray([SM_Le_x if n_x[i]>SM_S and n_x[i]<=SM_E else RM_Le_x for i in range(0,Nn)])
    Le_xa[0]=0
    #Element Change Indicator
    VM_N=([i for i in range(0,Nn) if n_x[i-1]<=SM_E and n_x[i]>SM_E])[0]


"""SUBSTRATE INPUT CALCULATIONS"""                                                            
"Indicators"
#Taper Approx. Indicator
TS_N=((([i for i in range(1,Nn) if n_x[i-1]<=TS and n_x[i]>TS])[0])-1 if T == "Y" else 0)
#Gap 1 Approx. Indicator
G1S_N=((([i for i in range(1,Nn) if n_x[i-1]<=G1S and n_x[i]>G1S])[0])-1 if G1 == "Y" else 0)
G1E_N=((([i if n_x[i-1]<G1E and n_x[i]>=G1E else Ne for i in range(1,Nn)])[0]) if G1 == "Y" else 0)                                                    
#Gap 2 Approx. Indicator
G2S_N=((([i for i in range(1,Nn) if n_x[i-1]<=G2S and n_x[i]>G2S])[0])-1 if G2 == "Y" else 0)
G2E_N=((([i if n_x[i-1]<G2E and n_x[i]>=G2E else Ne for i in range(1,Nn)])[0]) if G2 == "Y" else 0)                                                    
...
if Coating == "Y":
    "Indicators"
    #Taper Approx. Indicator
    TcS_N=((([i for i in range(1,Nn) if n_e_xc[i-1]<=TcS and n_e_xc[i]>=TcS])[0])-1 if Tc == "Y" else 0)
    #Gap 1 Approx. Indicator
    Gc1S_N=((([i for i in range(1,Nn) if n_e_xc[i-1]<=Gc1S and n_e_xc[i]>=Gc1S])[0])-1 if Gc1 == "Y" else 0)
    Gc1E_N=((([i if n_e_xc[i-1]<=Gc1E and n_e_xc[i]>=Gc1E else Nn for i in range(1,Nn)])[0])-1 if Gc1 == "Y" else 0) 
    #Gap 2 Approx. Indicator
    Gc2S_N=((([i for i in range(1,Nn) if n_e_xc[i-1]<=Gc2S and n_e_xc[i]>=Gc2S])[0])-1 if Gc2 == "Y" else 0)
    Gc2E_N=((([i if n_e_xc[i-1]<=Gc2E and n_e_xc[i]>=Gc2E else Nn for i in range(1,Nn)])[0])-1 if Gc2 == "Y" else 0) 
    "Coating Node Widths"
    #Pristmatic Widths (m)
    n_e_wc=np.zeros(Nn)
    ...
    if Spring == "Y" or Displacement == "Y" and Coating == "Y": 
...
"Part Angled Inputs"
PAr=np.zeros(Nn)                                                                        #Generate base Angle Array
if PAC == "Y":
    ...
    a=[i for i in range(PS_N,Nn) if n_e_xc[i-1]<PS and n_e_xc[i]>=PS][0]               #These find where the part angled section starts and ends with respect to the nodes it actually applies to and adjusts the element length accordingly.
    b=[i if n_e_xc[i-1]<PE and n_e_xc[i]>=PE else Ne for i in range(PE_N,Nn)][0]
    Pe_x=(Ls_A-n_x[a])/(b-a)
...
#Ytitles={ks_e():"Elemental Spring Constant (N/m)",ks_Along:"Along Cantilever Spring Constant (N/m)"}
...
if SensitivityAnalysis == "Y":
    ...
    Spec=gridspec.GridSpec(ncols=2, nrows=1, wspace=0.4, hspace=0.4)
    TL=SAGrid1.add_subplot(Spec[0,0]).plot(X,Y1,color="orange")
    TL=plt.xlim(0)
    TL=plt.title("Cantilever Spring Constant vs "+SI)
    TL=plt.xlabel(SI)
    TL=plt.ylabel("Spring Constant (N/m)")
    TR=SAGrid1.add_subplot(Spec[0, 1]).plot(X,Y2*1e+6,color="orange")
    TR=plt.xlim(0)
...
#            Le_xa[i]=Pe_x 
#    #Part Angled Element Angle Array
#            PAr[i]=np.deg2rad(PAd)
###########
